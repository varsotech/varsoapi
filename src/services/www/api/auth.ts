import {
  ...
  RegisterRequest,
  RegisterResponse,
} from "@varsotech/varsoapi/src/auth/requests"
...
export function useRegister() {
  return useMutation<RegisterRequest, AxiosResponse<RegisterResponse>>(
    async (req: RegisterRequest) => await axios.post("/api/v1/auth/register", req),
    { mutationKey: ["login"] }
  )
}
...
export function useEasyRegister() {
  return useMutation<EasyRegisterRequest, AxiosResponse<EasyRegisterResponse>>(
    async (req: EasyRegisterRequest) => await axios.post("/api/v1/auth/register/easy", req),
    { mutationKey: ["login"] }
  )
}
