...
//
//	client.Post.Query().
//		GroupBy(post.FieldAuthorUserUUID).
...
//
//	client.Post.Query().
//		Select(post.FieldAuthorUserUUID).
//		Scan(ctx, &v)
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withComments; query != nil {
		if err := pq.loadComments(ctx, query, nodes, nil,
			func(n *Post, e *Comment) { n.Edges.Comments = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
